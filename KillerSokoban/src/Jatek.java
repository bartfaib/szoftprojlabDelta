...
public class Jatek {
	...
	public void Kor(){
		...
		//Az aktu疝is munk疽t sz疥ontart� v疝toz�
		int aktivMunkas = 0;
		...
		//Mozgas kimenetel駭ek elt疵ol疽疸 szolg疝� v疝toz�.
		Kimenetel k = null;
		
		//Munkas 疝ltal m馮 megtehet� l駱駸ek sz疥a
		int megLephet = MaxLepes;
		
		//Jatek tare vagy v馮et 駻t-e v疝toz�
		boolean tart = true;
		
		//Volt-e 駻v駭yes parancs 
		boolean volt = false;
		...
		while(tart == true){
			...
			//Ha valamelyik iranyba mozgas utasitast kapott a jatek
				...
				//Ellenorzi hogy vege van e a jateknak a lepes ut疣 ha nm 駸 az adott munk疽 m疵 
				...
				if(JatekVege() == false){
					...
					if(megLephet == 0){
						...
						while(kov == false){
							...
							//Kovetkezo munkasra l駱 nelve a v疝toz ha v馮ig 駻 ele 疝l咜ja
							aktivMunkas += 1;
							if(aktivMunkas == munkasok.size())
								aktivMunkas = 0;
	...
	boolean JatekVege2(){
		...
		//Ha nincs elo munkas falsal ter vissza ami a jatek veg騁 jelenti
		if(elo == false)
			return true;
		...
		while(tolhato == false && i < mezok.size()){
			 ...
			 * emelette �res mezo ha igen akkor abb a mez kiindulva megn騷i az szes
			 * lehetseges utat am刕 nem tal疝 egy mez munk疽t vagy minden lehets馮es 偀 el nem
			 * fogyott. 
			 */
			int index = 0;
			while(index < 4){
