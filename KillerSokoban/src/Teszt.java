...
public class Teszt {
	...
	public void tesztMainMenu() {
		...
		System.out.println(
				"Megjegyzes: A tesztesetek kiirasa munkassal kezdodik, de a diagrammok nagyr̩sze szomszedlekerdezessel indul.");
	...
	private void DrawToFile(Mezo start, String fileName) throws FileNotFoundException, UnsupportedEncodingException { // start:
																														// mezo
																														// amirol
																														// indul
																														// a
		...
		PrintWriter writer = new PrintWriter(dir + "\\Maps\\Test\\Outputs\\" + fileName, "UTF-8"); // A
																									// fajlbairashoz
		List<Mezo> mezok; // Betoltjuk a mezoket egy listaba, ez alapjan keszul
							// majd egy 2D tomb
		mezok = new ArrayList<>();
		mezok.add(start); // Elso listaelemnek megadjuk az "m0" mezoket, ebbol
							// kiundulva epeti fel majd a teljes listat
		...
		// A tesztesethez tartozo osszes mezo felderitese (tipikusan nem lenne
		// szukseg mindegyik iranyra itt, de a teljes palya kirajzolasahoz jol
		// johet)
		Mezo m = mezok.get(0);
		...
		// Az elozo lepesben letrehozott tomb segitsegevel kirajzoljuk a
		// dolgokat.
					...
					if (palya[x][y].getLada() != null) {
						...
						for (int in = 0; in < munkasok.size(); in++) {
							if (munkasok.get(in) == mu)
								writer.print(in); // Munkas kirajzolasa
													// kapcsolon
						}
					} else {
